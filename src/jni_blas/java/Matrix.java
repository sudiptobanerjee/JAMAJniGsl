...
public class Matrix implements Cloneable, java.io.Serializable {
 private Matrix() {}
    ...
    */
    ...
    /* switch 
    public void Rswitch(int Layout, int m, int n, double[] x){
        if( Layout == Matrix.LAYOUT.RowMajor) {}
        else if( Latout == Matrix.LAYOUT.ColMajor){
            double [] tempx;
            System.arraycopy(x, 0, y, 0, m*n);
            int i, j;
            for(i = 0; i < m; i++)
                for( j = 0; j < n; j++)
                    x[i * ldout + j] = y[j * ldin + i];
        }
    }
*/
}
